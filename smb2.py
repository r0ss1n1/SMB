# @r0ss1n1
# SMBv1 SMB_COM_NEGOTIATE
# Charles Thomas Wallace Truscott
import socket
import time
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:

    host = "127.0.0.1"
    port = 445

    s.connect((host, port))
    s.sendall(b'')
    print(str(s.recv(4096)), 'ascii')
    time.sleep(3)
    s.sendall(b'')
    print(str(s.recv(4096)), 'ascii')
    time.sleep(3)
    s.sendall(b'')
    negotiate_session_1 = s.recv(4096)
    print(str(negotiate_session_1), 'ascii')
    sessionID = negotiate_session_1[44:52]
    print("sessionID ==" + str(sessionID))
    time.sleep(3)
    negotiate_session_2 = b''
    print(str(s.recv(4096)), 'ascii')
    time.sleep(3)
    s.sendall(b'')
    print(str(s.recv(4096)), 'ascii')
    time.sleep(3)
#    s.sendall(b'')
#    print(str(s.recv(4096)), 'ascii')
